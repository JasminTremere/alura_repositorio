...
with col_graf1:
    if not df_filtrado.empty:
        grafico_categoria = df_filtrado.groupby('Categoria')['Count'].mean().nlargest(10).sort_values(ascending=True).reset_index()
        fig_ead = px.bar(
            grafico_categoria,
            x='Categoria',
            y='Count',
...
with col_graf2:
    if not df_filtrado.empty:
        fig_pres_semi = px.bar(
            ...
            x='Categoria',
            y='Count',
            title='Distribuição de Categorias - Outros Pre',
            labels={'Categoria':'Categoria', 'Count':'Quantidade'}
        )
