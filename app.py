...
df_sup_ead = pd.read_csv('https://raw.githubusercontent.com/JasminTremere/alura_repositorio/main/Suporte_ead.csv', sep=';')
df_sup_pres_semi = pd.read_csv('https://raw.githubusercontent.com/JasminTremere/alura_repositorio/refs/heads/main/Suporte_pres_semi.csv', sep=';')
...
# Gráficos Relatório Outros EAD e Outros Presencial e Semipresencial 
 
col_graf1, col_graf2 = st.columns(2)
 
with col_graf1:
    if not df_ead_limpo.empty:
        grafico_categoria_ead = (
            df_ead_limpo.groupby('Categoria').size()
            .nlargest(10)
        ...
        fig_ead = px.bar(
            ...
            x='Categoria',
            y='Quantidade',
            title='Distribuição de Categorias - Outros EAD',
...
with col_graf2:
    if not df_pres_semi_limpo.empty:
        ...
        fig_pres_semi = px.bar(
            ...
            x='Categoria',
            y='Quantidade',
            title='Distribuição de Categorias - Outros Presencial e Semipresencial',
            labels={'Categoria': 'Categoria', 'Quantidade': 'Quantidade'}
        )
        fig_pres_semi.update_layout(title_x=0.1)
...
with col_graf4:
    if not df_sup_pres_semi_limpo.empty:
        grafico_sup_pres_semi = (
            df_sup_pres_semi_limpo.groupby('Categoria').size()
            .nlargest(10)
            .sort_values(ascending=True)
            .reset_index(name='Quantidade')
        )
        fig_sup_pres_semi = px.bar(
            ...
            x='Categoria',
            y='Quantidade',
            title='Distribuição de Categorias - Suporte Presencial e Semipresencial',
            labels={'Categoria': 'Categoria', 'Quantidade': 'Quantidade'}
        )
        fig_sup_pres_semi.update_layout(title_x=0.1)
        st.plotly_chart(fig_sup_pres_semi, use_container_width=True)
    else:
        st.warning("Nenhum dado para exibir no gráfico.")

 # Tabela Relatório Suporte EAD e Suporte Presencial e Semipresencial
...
with col_tab2:
    tabela_sup_pres_semi = df_sup_pres_semi_limpo.groupby('Categoria').size().reset_index(name='Quantidade')
